...
void read_2_c_and_c(Course course[] ,Classroom classroom[] , int count_courses ,int count_classrooms)	/*this function propose the course*/
{
	...
	{
		file >> classroom[i].id;
		file >> classroom[i].c_no;
		file >> classroom[i].capacity;
		classroom[i].student_inroom = 0;	
	}
	file.close();	
}

void read_3_lecturers_size(int & count_lecturers)	/*this function find the lecture size*/
{
	string array; 
	ifstream file;
	file.open("lecturers.txt");	/*there first open for oursecount the long*/
...
void read_4_fill_lecturer(Lecturer lecturer[] ,int count_lecturers)		/*this function propose the lecture*/
{
	string array;
	int i ,j , number;
	...
	for (i = 0; i < count_lecturers; ++i)		/*assign lecture*/
	{
		file >> number;
